 ...
 OnShowFiles: () => void;
    ...
    const filePanel = () => {
     if(props.isDownload && props.fileIds.length > 0) {
       const fileName = props.downloadFileName ? `${props.downloadFileName}.tar.gz` : 'nexus-download.tar.gz';
       return (
         <div className="download">
           <a href={downloadUrl}  style={linkStyle} ref={refContainer} download={fileName}>download</a> 
            <Button size='small' onClick={() => { setLoading(1);}} icon={icons[isLoading]} loading={isLoading === 1} onBlur={() => {setLoading(0)}}>
    ...
    React.useEffect(() => {
      if(props.fileIds.length > 0 && isLoading === 1) {
        ...
        nexus.Archive.create(org, proj,  { 'resources' : resources } ).then((archive: any) => {
          if(archive) {
